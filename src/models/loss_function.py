...
class AsymmetricFocalLoss(nn.Module):
    ...
    #     loss = torch.mean(torch.sum(torch.stack([back_ce, fore_ce], axis=-1), axis=-1))
        ...
        # Calculate cross-entropy component for both classes
        cross_entropy = -y_true * torch.log(y_pred) - (1 - y_true) * torch.log(1 - y_pred)
        
        # Calculate loss for the background class (non-target)
        back_ce = torch.pow(1 - y_pred, self.gamma) * cross_entropy * (1 - self.delta)
        
        # Calculate loss for the foreground class (target)
        fore_ce = torch.pow(y_pred, self.gamma) * cross_entropy * self.delta
...
class AsymmetricFocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        # # Average class scores
        # loss = torch.mean(torch.stack([back_dice,fore_dice], axis=-1))
        dice_score = (tp + self.epsilon) / (tp + self.delta * fn + (1 - self.delta) * fp + self.epsilon)

        # For a single-channel output, calculate foreground and background losses directly
        background_loss = 1 - dice_score  # Background class is the inverse of foreground
        foreground_loss = (1 - dice_score) * torch.pow(1 - dice_score, -self.gamma)  # Enhance foreground

        # Combine losses
        loss = torch.mean(foreground_loss + background_loss)  # Averaging the losses
        return loss
