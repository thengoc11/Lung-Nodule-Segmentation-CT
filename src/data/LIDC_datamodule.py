...
import time
import csv
from data.util.transform import make_transform

class LIDC_IDRI_Dataset(Dataset):
    def __init__(self, nodule_path, clean_path, mode, img_size=[128, 128], transform=None):
        ...
        print(f"{mode}: {len(self.file_list)}")
...
class LIDCDataModule(LightningDataModule):
    def __init__(
        ...
        # file_nodule_list = file_nodule_list[:10]

        # file_clean_list = file_clean_list[:10]
        # print(file_clean_list,file_nodule_list)
        ...
        self.data_train = LIDC_IDRI_Dataset(nodule_train, clean_train, mode="train", img_size=img_size, transform=train_transform)

        self.data_val = LIDC_IDRI_Dataset(nodule_val, clean_val, mode="valid", img_size=img_size, transform= train_transform)

        self.data_test = LIDC_IDRI_Dataset(nodule_test, clean_test, mode="test", img_size=img_size, transform=test_transform)
